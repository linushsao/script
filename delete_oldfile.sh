...
if [ "$CAPACITY_LIMIT" == "" ]
then
    CAPACITY_LIMIT=90   # default limit
fi
...
then
    ...
    do
        ...
        then
            if rm -f $FILE
            then
                echo "$DTIME Deleted $FILE" >> /home/linus/log/DELETE_OLD_FILES.log
    ...
    else echo "$DTIME have enougnt hd space reaches to "$CAPACITY >> /home/linus/log/DELETE_OLD_FILES.log
fi
